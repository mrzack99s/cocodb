...
func (s *Server) registerStaticFileServer(mux *http.ServeMux) {
	...
	distDir := filepath.Join("dashboard", "frontend", "dist")
	if info, err := os.Stat(distDir); err == nil && info.IsDir() {
		fsHandler := http.FileServer(http.Dir(distDir))
		mux.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
			if stringsHasPrefix(r.URL.Path, "/api") || r.URL.Path == "/metrics" {
				http.NotFound(w, r)
				return
			}
	...
	mux.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`<!DOCTYPE html><html><body style="font-family:sans-serif;background:#09090b;color:#e4e4e7;padding:40px;text-align:center;"><h2>CoCoDB Metrics Dashboard API Active</h2><p style="color:#a1a1aa">Frontend UI is available at <a style="color:#10b981" href="http://localhost:5174">http://localhost:5174</a> or build via <code>npm run build</code> in <code>dashboard/frontend</code>.</p></body></html>`))
	})
}
